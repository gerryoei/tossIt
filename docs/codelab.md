...
In this codelab you will build a tossItem recommendation app on Android backed by Cloud Firestore. You will learn how to:
...
The main model object in our app is a tossItem (see `model/Restaurant.java`). Firestore data is split into documents, collections, and subcollections. We will store each tossItem as a document in a top-level collection called `"restaurants"`. To learn more about the Firestore data model, read about documents and collections in [the documentation](https://firebase.google.com/docs/firestore/data-model).
...
```
    private void onAddItemsClicked() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            Restaurant tossItem = RestaurantUtil.getRandom(this);

            // Add a new document to the restaurants collection
            restaurants.add(tossItem);
        }
    }
```
...
Now go back to the Firebase console and edit one of the tossItem names. You should see it change in the app almost instantly!


## Sort and filter data
Duration: 5:00

The app currently displays the top-rated restaurants across the entire collection, but in a real tossItem app the user would want to sort and filter the data. For example the app should be able to show "Top seafood restaurants in Philadelphia" or "Least expensive pizza".
...
The query could not be completed on the backend because it requires an index. Most Firestore queries involving multiple fields (in this case price and bid) require a custom index. Clicking the link in the error message will open the Firebase console and automatically prompt you to create the correct index:
...
If you've made it this far, you have now built a fully functioning tossItem recommendation app on Firestore!  You can now sort and filter restaurants in real time. In the next few sections we add more features and security to the app.
...
So far we have stored all tossItem data in a top-level collection called "restaurants". When a user rates a tossItem we want to add a new `Rating` object to the restaurants. For this task we will use a subcollection. You can think of a subcollection as a collection that is attached to a document. So each tossItem document will have a ratings subcollection full of bid documents. Subcollections help organize data without bloating our documents or requiring complex queries.
...
Adding a `Rating` to the proper subcollection only requires calling `.add()`, but we also need to update the `Restaurant` object's average bid and number of ratings to reflect the new data. If we use separate operations to make these two changes there are a number of race conditions that could result in stale or incorrect data.

To ensure that ratings are added properly, we will use a transaction to add ratings to a tossItem. This transaction will perform a few actions:

*   Read the tossItem's current bid and calculate the new one
*   Add the bid to the subcollection
*   Update the tossItem's average bid and number of ratings
    ...
    private Task<Void> addRating(final DocumentReference restaurantRef,
        ...
        return mFirestore.runTransaction(new Transaction.Function<Void>() {
            ...
            public Void apply(Transaction transaction)
                ...
                Restaurant tossItem = transaction.get(restaurantRef)
...
Now **Run** the app again and click on one of the restaurants, which should bring up the tossItem detail screen. Click the **+** button to start adding a review. Add a review by picking a number of stars and entering some text.

![Firestore-Android11](img/Firestore-Android11.png)

Hitting **Submit** will kick off the transaction. When the transaction completes, you will see your review displayed below and an update to the tossItem's review count:

![Firestore-Android12](img/Firestore-Android12.png)

Congrats!  You now have a social, local, mobile tossItem review app built on Cloud Firestore. I hear those are very popular these days.
...
These rules restrict access to ensure that clients only make safe changes. For example updates to a tossItem document can only change the ratings, not the name or any other immutable data. Ratings can only be created if the user ID matches the signed-in user, which prevents spoofing.
...
The tossItem app in this codelab was based on the "Friendly Eats" example application. You can browse the source code for that app [here](https://github.com/firebase/quickstart-android).
